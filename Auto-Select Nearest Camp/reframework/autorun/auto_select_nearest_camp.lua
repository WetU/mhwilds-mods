...
local distance = sdk.find_type_definition("via.MathEx"):get_method("distance(via.vec3, via.vec3)")
...
local function get_index_of_nearest_start_point(target_pos, start_point_list)
  ...
  for index, start_point in ipairs(start_point_list._items) do
    ...
    local d2 = distance:call(nil, beacon_pos, target_pos)
